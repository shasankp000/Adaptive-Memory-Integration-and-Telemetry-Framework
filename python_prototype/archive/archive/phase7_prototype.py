...
class AdaptivePoisonRegister:
    """
    v7 = v6 telemetry + adaptive response.

    Baseline mode exposes BASE_DECOYS decoy buffers. When telemetry sees a stable
    cadence that resembles a polling actor, the register enters poison mode for
    POISON_HOLD_EPOCHS epochs and increases decoy density up to MAX_DECOYS.

    The trigger is intentionally conservative in the prototype:
      - at least SUSPICION_HIT_THRESHOLD telemetry hits observed
      - running mean inter-hit delta inside [300 ms, 700 ms]

    This should activate cleanly with SIMULATE_OBSERVER=1 because the canary is
    mutated on a regular 500 ms schedule. In baseline mode without that observer,
    only the self-write/self-scrub cadence is present, so poison may also trigger;
    that is acceptable for the prototype because the focus is response plumbing.
    """
...
entityRegister = AdaptivePoisonRegister()
...
def gameloop():
    try:
        ...
        print(f"VERSION:            7  (v1+v2+v3+v4+v5+v6 + adaptive poisoning)")
        ...
        while i < 600:
            ...
            if second_counter - current_second == 20:
                ...
                poison   = int(entityRegister.poison_active(epoch))
                print(
                    f"[swap] tick={tick} epoch={epoch} "
                    f"real_addr={entityRegister.shared_address()} "
                    f"decoys={entityRegister.decoy_count()} "
                    f"poison={poison} activations={entityRegister.poison_activations()} "
